...
std::filesystem::path SupersonicMarioPluginDataFolder;
...
/// <param name="mutator"><see cref="SupersonicMarioPlugin::Mutator"/> to print the values from</param>
/// <param name="error">Error to prepend</param>
...
/// <param name="mutator"><see cref="SupersonicMarioPlugin::Mutator"/>'s to get the values from</param>
...
void SupersonicMarioPlugin::savePreset(const std::string& presetName)
{
    ...
    if (presetFile.is_open()) {
        presetFile << "// This preset has been autogenerated by Supersonic Mario Plugin\n";
        for (const GameSetting& mutator : mutators) {
...
void SupersonicMarioPlugin::OnLoad()
{
    ...
    SupersonicMarioPluginDataFolder = gameWrapper->GetDataFolder() / L"SupersonicMarioPlugin";
    if (!exists(SupersonicMarioPluginDataFolder)) {
        create_directory(SupersonicMarioPluginDataFolder);
    }
    ...
    ConstantsConfig = std::make_unique<SMPConfig>(DEFAULT_CONSTANTS_CONFIG_URL);
    ...
    /* Init Modules */
    SupersonicMarioPluginModule::supersonicMarioPlugin = this;
...
/// <summary>Registers CVars for Supersonic Mario Plugin.</summary>
void SupersonicMarioPlugin::registerCVars()
{
    ...
    cvarManager->registerCvar("sm_gui_keybind", DEFAULT_GUI_KEYBIND, "Keybind for the gui");
...
/// <summary>Registers notifiers for Supersonic Mario Plugin.</summary>
void SupersonicMarioPlugin::registerNotifiers()
{
    RegisterNotifier("rp", [this](const std::vector<std::string>& arguments) {
        parseArguments(arguments);
    }, "Parses commands to interact with Supersonic Mario Plugin.", PERMISSION_ALL);
    ...
    RegisterNotifier("rp_change_keybind", [this](const std::vector<std::string>& arguments) {
        ...
        if (arguments.size() >= 2) {
            ...
            key = cvarManager->getCvar("sm_gui_keybind").getStringValue();
        }

        const std::string command = "togglemenu " + GetMenuName();
        cvarManager->setBind(key, command);
        BM_LOG("Set {:s} to {:s}", quote(key), quote(command));
    }, "Adds a keybind for " + quote("togglemenu " + GetMenuName()) + " as $sm_gui_keybind or given argument.",
    PERMISSION_ALL);
...
/// <summary>Register hooks for Supersonic Mario Plugin.</summary>
