// SMPConfig.cpp
// Online config for Supersonic Mario Plugin.
//
// Author:        Stanbroek
...
/// <summary>Parses game settings json string into Supersonic Mario Plugin variables.</summary>
/// <param name="data">Json string with game settings</param>
/// <param name="SupersonicMarioPlugin">Supersonic Mario Plugin class to store the result in</param>
/// <returns>Bool with if the game settings where parsed successfully</returns>
bool SMPConfig::ParseGameSettings(const std::string& data, SupersonicMarioPlugin* supersonicMarioPlugin)
{
    ...
    try {
        parseGameModes(doc, supersonicMarioPlugin->gameModes);
        parseBotDifficulties(doc, supersonicMarioPlugin->botDifficulties);
        parseAvailableMaps(doc, supersonicMarioPlugin->maps);
        parseAvailableColors(
            doc, supersonicMarioPlugin->customColorHues, supersonicMarioPlugin->customColors, supersonicMarioPlugin->clubColorHues,
            supersonicMarioPlugin->clubColors, supersonicMarioPlugin->defaultBluePrimaryColor, supersonicMarioPlugin->defaultBlueAccentColor,
            supersonicMarioPlugin->defaultOrangePrimaryColor, supersonicMarioPlugin->defaultOrangeAccentColor);
        parseAvailableMutators(doc, supersonicMarioPlugin->mutators);
    }
...
bool SMPConfig::ParseRumbleItems([[maybe_unused]] const std::string& data, [[maybe_unused]] CrazyRumble* crazyRumble)
{
    return false;
}
...
std::future<std::pair<bool, std::string>> SMPConfig::RequestGameSettingConstants()
{
...
std::future<std::pair<bool, std::string>> SMPConfig::RequestRumbleConstants()
{
    return std::async([this]() {
...
void SMPConfig::parseGameModes(simdjson::ondemand::document& doc, SupersonicMarioPlugin::GameSetting& gameModes)
{
...
void SMPConfig::parseBotDifficulties(simdjson::ondemand::document& doc, SupersonicMarioPlugin::GameSetting& botDifficulties)
{
...
void SMPConfig::parseAvailableColors(simdjson::ondemand::document& doc, int& customColorHues,
...
void SMPConfig::parseAvailableMutators(simdjson::ondemand::document& doc,
...
/// <summary>Requests and parses Supersonic Mario Plugin config.</summary>
/// <returns>Future with if the request was successful</returns>
std::future<bool> SMPConfig::requestConfig()
{
