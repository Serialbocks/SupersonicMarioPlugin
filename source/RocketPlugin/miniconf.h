...
namespace miniconf
{
    ...
    Config::LogLevel Config::checkFormat()
    {
        ...
        for (auto&& opt : _options) {
            ...
            if (!o.required() && o.defaultValue().isEmpty()) {
                log(LogLevel::ERR, o.flag(), "default value is not defined");
                errorLv = worseLevel(errorLv, LogLevel::ERR);
            }
            for (auto& opt2 : _options) {
                Option& o2 = opt2.second;
                if ((o.flag() != o2.flag()) && (o.shortflag() == o2.shortflag()) && !(o.shortflag().empty())) {
                    log(LogLevel::ERR, o.flag(), "duplicate short flags (" + o2.shortflag() + ")");
                    errorLv = worseLevel(errorLv, LogLevel::ERR);
                }
            }
            // check for warnings
    ...
    Config::LogLevel Config::validate()
    {
        ...
        for (auto&& val : _optionValues) {
            if (val.second.isEmpty()) {
                log(LogLevel::ERR, val.first, "option contains invalid value");
                errorLv = worseLevel(errorLv, LogLevel::ERR);
            }
        }
        ...
        for (auto&& opt : _options) {
            if (!contains(opt.first) && !opt.second.hidden()) {
                log(LogLevel::ERR, opt.first, "option is undefined");
                errorLv = worseLevel(errorLv, LogLevel::ERR);
            }
        }
        return errorLv;
    ...
    bool Config::parse(int argc, char** argv)
    {
        ...
        if (checkFormatResult >= LogLevel::ERR && _logLevel <= LogLevel::ERR) {
            log();
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (currentTokenType == TokenType::UNKNOWN) {
                log(LogLevel::ERR, std::string(argv[i]), "unknown input");
            }
        ...
        if (validateResult >= LogLevel::ERR && _logLevel <= LogLevel::ERR) {
            log();
            printf("\nFatal Error: Option format validation failed, abort.\n\n");
            return false;
        }
